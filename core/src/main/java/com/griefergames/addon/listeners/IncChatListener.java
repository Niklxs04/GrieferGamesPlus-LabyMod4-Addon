...
public class IncChatListener {
    ...
    public void ChatListener(ChatReceiveEvent event){
        if( GrieferGamesAddon.getSharedInstance().configuration().getZauberer().get()) {
            if (event.chatMessage().getOriginalPlainText()
                .contains("[GrieferGames] Ein Zauberer ist auf dem Grundstück")) {
                    ...
                    if (split.matches("-?\\d.*")) {
                        ...
                        for (int i = 0; i <= 7; i++) {
                            ...
                            if (i == 3) {
                                Laby.references().chatExecutor().displayClientMessage(
                                    I18n.getTranslation(
                                        "griefergames-plus.IncChat.ZaubererErschienen") + split);
                            } else if (i == 0 || i == 7) {
                                Laby.references().chatExecutor().displayClientMessage(
                                    "§4§l---------------------------------------------------------");
                            } else if (i == 4) {
                                Laby.references().chatExecutor().displayClientMessage(
                                    I18n.getTranslation("griefergames-plus.IncChat.directTP"));
                            } else {
                                Laby.references().chatExecutor().displayClientMessage(" ");
                            }
                        }
                        Notification.builder()
                            .title(Component.text(Objects.requireNonNull(I18n.getTranslation(
                                    "griefergames-plus.IncChat.ZaubererErschienen")),
                                NamedTextColor.BLUE))
